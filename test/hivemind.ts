...
describe("HivemindAPI", function () {
  ...
  it("getRankedPosts", async () => {
    const r = await liveClient.hivemind.getRankedPosts({
      limit: 1,
      sort: "trending",
      tag: "",
      observer: "",
  ...
  it("getCommunity", async () => {
    const r = await liveClient.hivemind.getCommunity({
      name: "hive-148441",
      observer: "",
    });
  ...
  it("getAccountNotifications", async () => {
    const r = await liveClient.hivemind.getAccountNotifications({
      account: "acidyo",
      limit: 2,
    });
  ...
  it("listAllSubscriptions", async () => {
    const r = await liveClient.hivemind.listAllSubscriptions({
      account: "acidyo",
    });
